	...
	virtual int classify(Packet* p) {
		int cl;
                ...
                if (perflow_) {
                        ...
                        int ms_ = (h->flowid() >> (32 - h->ttl()) + h->prio()) % (maxslot_ + 1);
                        int fail = ms_;
                        do {
                                cl = ms_++;
                                ms_ %= (maxslot_ + 1);
                        ...
                        if (debug_) {
                                printf("fid = %d, ttl = %d, prio = %d, ms_= %u, cl = %d\n", \
                                       h->flowid(), h->ttl(), h->prio(), ms_, cl);
                        }
...
private:
	int ns_;       //next slot to be used
        int perflow_;   //perflow ECMP (1) or not (0)
        int debug_;     //whether print necessary debug information
};
